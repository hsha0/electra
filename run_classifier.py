    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(per_example_loss, label_ids, logits, is_real_example):
        if regression:
            ...
            mcc.update_state(tf.cast(label_ids, tf.float32), tf.cast(predictions, tf.float32))
            mcc = tf.compat.v1.metrics.mean(mcc.result())
...
def main(_):
  ...
  if FLAGS.max_seq_length > electra_config.max_position_embeddings:
    raise ValueError(
        ...
        (FLAGS.max_seq_length, electra_config.max_position_embeddings))

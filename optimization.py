...
def create_lamb_optimizer(loss, init_lr, total_num_train_steps, num_warmup_steps, use_tpu, weight_decay=0.0, layer_wise_lr_decay=1.0, part='disc'):
  ...
  for i, grad in enumerate(grads):
      layer_wise_lr_multiplier.append(tf.ones(shape=grad.shape) * layer_wise_lr_decay ** i)

  #layer_wise_lr_multiplier = tf.reshape(layer_wise_lr_multiplier, [-1,1,1])
  print(layer_wise_lr_multiplier)
  print(grads)

  #grads = tf.multiply(grads, layer_wise_lr_multiplier)
  grads = tf.map_fn(fn=lambda x: tf.multiply(x, layer_wise_lr_decay ** grads.index(x)), elems=tf.constant(grads))
  print(grads)
  sys.exit()
...
def create_adam_optimizer(loss, init_lr, total_num_train_steps, num_warmup_steps, use_tpu, weight_decay=0.0):
    global_step = tf.compat.v1.train.get_or_create_global_step()
